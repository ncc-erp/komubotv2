 ...
 * @param {object} client The discord client instance
 */
export const getServersList = async (client) => {
 ...
 * @param {object} client The discord client instance
 */
export const createClientVars = async (client) => {
 ...
 */
export const resolveCategory = async function (args, client: any = {}) {
...
export const checkConfig = async (config) => {
  ...
  let error;
  if ((process.version.slice(1).split(".")[0] as any) < 12) {
  ...
  if (!config.devMode || (typeof config.devMode as any) !== Boolean) {
    console.error(
